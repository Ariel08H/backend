...
/*
  Reads the verified Clerk identity.

  The user ID comes from the verified Clerk token.
  The email and name are fetched from Clerk's backend API,
  not accepted from req.body.
*/
...
router.post('/create', async (req, res) => {
  try {
    ...
    if (error) {
      console.error('RPC error while creating event:', error.message);
...
router.put('/update/:id', async (req, res) => {
  try {
    ...
    const query = `
      ...
      WHERE id = ${sqlValue(id)}
      AND "user_email" = ${sqlValue(authenticatedUser.email)}
      RETURNING *, "date" AS last_date
    `;
    ...
    console.error('Server error while updating event:', err);
